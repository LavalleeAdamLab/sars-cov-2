...
public class Loader {
	...
	public static ArrayList<Interaction> loadInteractionRepository(String inputRepositoryFile, boolean runOnComputeCanada) {
		...
		for (String inter : InteractionToNumber.keySet()) {
			...
			Interaction inter1 = new Interaction(interaction_identifier[0], interaction_identifier[1], interaction_identifier[2], interaction_identifier[3]); // initialize new interaction object
	...
	public static ArrayList<Annotation> importAnnotationGo(String inputFile, ArrayList<Protein> networkProteinList, boolean runComputeCanada) {
		...
		try {
			...
			int goCount = 0;
			while (line != null) {
				if(goCount < 5000000 && goCount%1000000 == 0) {
					System.out.println("go term = " +goCount);
				}
				if(goCount>5000000 && goCount%1000 == 0) {
					System.out.println("go term = " +goCount);
				}
				String[] col = line.split("\t");

				String term = col[0]; // first column holds GO terms (index = 0)
				String protein_ids = col[7]; // 7th column holds gene IDs (proteins IDs) (index = 6)
				String protein_symbols = col[8]; // 8th column holds gene symbols (protein symbols) (index = 7)
				...
				for (int i = 0; i < proteinIDList.length; i++) { // search for all IDs associated to the GO term
					...
					while (!foundProt && (protCounter < networkProteinList.size())) {
						...
						if (networkProteinList.get(protCounter).getProteinName().equals(proteinSymbolsList[i])) {
