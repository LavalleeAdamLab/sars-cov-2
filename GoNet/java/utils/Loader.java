...
public class Loader {
	...
	public static ArrayList<Interaction> loadInteractionRepository(String inputRepositoryFile, boolean runOnComputeCanada) {
		...
		for (String inter : InteractionToNumber.keySet()) {
			...
			Interaction inter1 = new Interaction(interaction_identifier[0], interaction_identifier[1], Integer.parseInt(interaction_identifier[2]), Integer.parseInt(interaction_identifier[3])); // initialize new interaction object
	 ...
	 */
	public static void loadProteinFoldChangeData(String foldChangeDataFile, ArrayList<Protein> proteinsInNetworkList, boolean testHer2vsTN, boolean runOnComputeCanada) {
		...
		try {
			...
			ArrayList<Integer> subtype1indexes = new ArrayList<Integer>();

			if(testHer2vsTN) {
				subtype1indexes = her2Indexes;
			} else {
				subtype1indexes = erprIndexes;
			}
			...
			while (line != null) {
				...
				col = line.split("\t");
				try {
					double avSubtype1 = Calculator.computeAvg(col, subtype1indexes);
					double avTN = Calculator.computeAvg(col, tnIndexes);

					// expression = fold change for Her2
					double foldChange = avSubtype1 / avTN;
	...
	public static ArrayList<Annotation> importAnnotationGo(String inputFile, ArrayList<Protein> networkProteinList) {
		...
		try {
			...
			while (line != null) {
				...
				String protein_ids = col[6]; // 7th column holds gene Entrez IDs (proteins IDs) (index = 6)
				...
				String[] proteinEntrezIDList = protein_ids.split("\\|");
				String[] proteinSymbolsList = protein_symbols.split("\\|");  // split for names of individual proteins
				 ...
				 * within the network. */
				ArrayList<Integer> goProteinsInNetworkList = new ArrayList<Integer>();
				...
				for (int i = 0; i < proteinEntrezIDList.length; i++) { // search for all IDs associated to the GO term
					...
					while (!foundProt && (protCounter < networkProteinList.size())) {
						...
						if (networkProteinList.get(protCounter).getProteinEntrezId() == Integer.parseInt(proteinEntrezIDList[i])) {
							goProteinsInNetworkList.add(Integer.parseInt(proteinEntrezIDList[i])); // save protein ID number
				...
				Double goPrevalence = goProteinsInNetworkList.size() / (double) (proteinEntrezIDList.length);

				if (goPrevalence != 0.0 && goProteinsInNetworkList.size() > 2 && goProteinsInNetworkList.size() < 1000) {
					...
					if(goProteinsInNetworkList.size()>protCount) {
						protCount = goProteinsInNetworkList.size();
					}
				}
			...
			input.close();
			System.out.println("Number of proteins to sample : " + protCount);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return goTermsList;
		...
		try {
			...
			while(line!=null) {
				if (!line.equals("")) {
					...
					if (col[0].equals("TPD") && col[4].equals("Frequency") || col[0].equals("Done")) {
	...
	public static double[][] loadMonteCarloDistributions(String distributionInputFile, int nProtToSampleLowerBound, int nProtToSampleUpperBound, String normalDistributionParametersFile) {
		...
		double[][] normalDistributionParameters = new double[nProtToSampleUpperBound-nProtToSampleLowerBound+1][2];
		...
		try {
			...
			while(line!=null) {
					...
					}else if(line.isEmpty()||line.contains("Done")) {
	...
	public static void loadShuffledGoAnnotations(ArrayList<Annotation> shuffledAnnotationGoList, String shuffled_annotationGo_inputFile) {
		...
		try {
			...
			while(line != null) {
				...
				ArrayList<Integer> proteinIDsList = new ArrayList<Integer>(); 
				for(int i=0; i<proteinIDs.length; i++) {
					proteinIDsList.add(Integer.parseInt(proteinIDs[i]));
				}
