...
public class Protein {
	...
	private final static double DEFAULT_FOLDCHANGE = 1.0;
		...
		private int proteinEntrezId;
		private int numberOfInteractions;
		private double fold_change;

		// constructor
		public Protein(String _name, int protId) {
			proteinName = _name;
			proteinEntrezId = protId;
		...
		public int getProteinEntrezId() {
			return this.proteinEntrezId;
		}
		...
		public void setId(ArrayList<Interaction> InteractionList) {
			...
			while (foundID == false) {
				...
				if (inter.getProtein1() == this.proteinName) {
					proteinEntrezId = inter.getID1();
					j = 0;
					foundID = true;
				} else if (inter.getProtein2() == this.proteinName) {
					proteinEntrezId = inter.getID2();
					j = 0;
					foundID = true;
				} else {
					j++;
